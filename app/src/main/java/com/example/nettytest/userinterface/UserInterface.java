...
public class UserInterface {
    ...
    public final static int CALL_SERVER_BACKUP_DEVICE = 91;
    ...
    public final static int NOTIFY_USER_MSG = 1;
    public final static int NOTIFY_MAIN_SERVER_ACTIVE = 100;
    ...
    public static void InitParam(String fileName){
        PhoneParam.InitPhoneParam(fileName);
    }
    ...
    public static OperationResult AddDeviceOnServer(String id,int type,int netMode,String area){
        OperationResult result = new OperationResult();
        int failReason;
        int typeInServer = PhoneDevice.UNKNOW_CALL_DEVICE;
        int netType;
        ...
        switch(type){
            case CALL_BED_DEVICE:
                typeInServer = PhoneDevice.BED_CALL_DEVICE;
                break;
            case CALL_DOOR_DEVICE:
                typeInServer = PhoneDevice.DOOR_CALL_DEVICE;
                break;
            case CALL_NURSER_DEVICE:
                typeInServer = PhoneDevice.NURSE_CALL_DEVICE;
                break;
            case CALL_DOCTOR_DEVICE:
                typeInServer = PhoneDevice.DOCTOR_CALL_DEVICE;
                break;
            case CALL_TV_DEVICE:
                typeInServer = PhoneDevice.TV_CALL_DEVICE;
                break;
            case CALL_CORRIDOR_DEVICE:
                typeInServer = PhoneDevice.CORRIDOR_CALL_DEVICE;
                break;
            case CALL_DOOR_LIGHT_DEVICE:
                typeInServer = PhoneDevice.DOOR_LIGHT_CALL_DEVICE;
                break;
            case CALL_WHITE_BOARD_DEVICE:
                typeInServer = PhoneDevice.WHITE_BOARD_DEVICE;
                break;
            case CALL_EMERGENCY_DEVICE:
                typeInServer = PhoneDevice.EMER_CALL_DEVICE;
                break;
            case CALL_SERVER_BACKUP_DEVICE:
                typeInServer = PhoneDevice.SERVER_BACKUP_DEVICE;
                break;
        }

        if(callServer!=null&& typeInServer!=PhoneDevice.UNKNOW_CALL_DEVICE)
            failReason = callServer.AddBackEndPhone(id, typeInServer,netType,area);
        else
            failReason = FailReason.FAIL_REASON_NOTSUPPORT;
        if(failReason!= FailReason.FAIL_REASON_NO){
            result.result = OperationResult.OP_RESULT_FAIL;
            result.reason = failReason;
        }
    ...
    public static int UpdateAreaDevices(String areaId, ArrayList<UserDevice> devList) {
    	HandlerMgr.UpdateAreaDevices(areaId,devList);
    	return 0;
    }
        ...
        switch(type) {
            ...
            case CALL_SERVER_BACKUP_DEVICE:
                if(PhoneParam.serverActive){
                    ...
                    HandlerMgr.CreateBackupServer(ID, netType);
                }
                break;
            default:
                result.result = OperationResult.OP_RESULT_FAIL;
    ...
    public static OperationResult BuildAlert(String id,int type){
        ...
        if(type>CommonCall.ALERT_TYPE_ENDED-CommonCall.ALERT_TYPE_BEGIN){
            result.result =OperationResult.OP_RESULT_FAIL;
            result.reason = FailReason.FAIL_REASON_NOTSUPPORT;
            PrintLog("Create type %d Alert From %s  Fail, Reason is %s",type,id,FailReason.GetFailName(result.reason));
        }else{
    ...
    public static OperationResult BuildCall(String id, String peerId, int type){
        ...
        if(callid!=null){
            ...
            result.result = OperationResult.OP_RESULT_FAIL;
            result.reason = FailReason.FAIL_REASON_NOTSUPPORT;
            PrintLog("Create Call From %s to %s Fail, Reason is %s",id,peerId,FailReason.GetFailName(result.reason));
        }
    ...
    public static OperationResult EndCall(String devid, String callid){
        ...
        if(operationCode== ProtocolPacket.STATUS_OK)
            PrintLog("End Call %s by %s success",callid,devid);
        else
            PrintLog("End Call %s by %s Fail, Reason is %s",callid,devid,FailReason.GetFailName(result.reason));
    ...
    public static OperationResult EndAlert(String devid,String alertId){
        ...
        if(operationCode== ProtocolPacket.STATUS_OK)
            PrintLog("End Alert %s by %s success",alertId,devid);
        else
            PrintLog("End Alert %s by %s Fail, Reason is %s",alertId,devid,FailReason.GetFailName(result.reason));
    ...
    public static OperationResult EndCall(String devid,int callType){
        ...
        if(operationCode== ProtocolPacket.STATUS_OK)
            PrintLog("End Call type %d by %s success",callType,devid);
        else
            PrintLog("End Call type %d by %s Fail, Reason is %s",callType,devid,FailReason.GetFailName(result.reason));
    ...
    public static OperationResult AnswerCall(String devid, String callid){
        ...
        result.callID = callid;
        if(operationCode== ProtocolPacket.STATUS_OK)
            PrintLog("Answer Call %s by %s success",callid,devid);
        else
            PrintLog("Anwser Call %s by %s Fail, Reason is %s",callid,devid,FailReason.GetFailName(result.reason));
        return result;
    }
    ...
    public static OperationResult StopVideo(String devid,String callid){
        ...
        result.callID = callid;
        if(operationCode== ProtocolPacket.STATUS_OK)
            PrintLog("Stop Video in Call %s by %s success",callid,devid);
        else
            PrintLog("Stop Video in Call %s by %s Fail, Reason is %s",callid,devid,FailReason.GetFailName(result.reason));
        return result;
    }
    ...
    public static OperationResult StartVideo(String devid,String callid){
        ...
        result.callID = callid;
        if(operationCode== ProtocolPacket.STATUS_OK)
            PrintLog("Start Video in Call %s by %s success",callid,devid);
        else
            PrintLog("Start Video in Call %s by %s Fail, Reason is %s",callid,devid,FailReason.GetFailName(result.reason));
        return result;
    }
    ...
    public static OperationResult AnswerVideo(String devId,String callId){
        ...
        result.callID = callId;
        if(operationCode== ProtocolPacket.STATUS_OK)
            PrintLog("Answer Video in Call %s by %s success",callId,devId);
        else
            PrintLog("Answer Video in Call %s by %s Fail, Reason is %s",callId,devId,FailReason.GetFailName(result.reason));
        return result;
    }
    ...
    public static int GetDeviceType(String name){
        ...
        else if(name.compareToIgnoreCase("backupServer")==0)
            type = CALL_SERVER_BACKUP_DEVICE;
        else
