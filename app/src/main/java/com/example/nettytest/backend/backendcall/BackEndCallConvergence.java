...
public class BackEndCallConvergence {
    ...
    public BackEndCallConvergence(BackEndPhone caller, InviteReqPack pack) {
        ...
        if(CommonCall.IsAlertCall(pack.callType)){
            ...
            if(alertConfig==null){
                ...
                pack.nameType = alertConfig.devType;
    ...
    public int UpdateCallListen(String devId){
        ...
        //skip the caller, 
        if(callerNum.compareToIgnoreCase(devId)==0)
            return -1;
...
//                        LogWork.Print(LogWork.BACKEND_CALL_MODULE, LogWork.LOG_ERROR, "BackEnd Set dev %s  for Call %s Ring State", packet.sender,inviteCall.callID);
    ...
    public boolean CheckInviteEnable(BackEndPhone phone){
        ...
        if(!phone.isReg)
            ...
            if(CommonCall.IsAlertCall(inviteCall.callType))
                result = true;
        }
        return result;
    }
                    ...
                    case CommonCall.CALL_TYPE_ASSIST:
                        switch(phone.type){
                            ...
                            case BackEndPhone.TV_CALL_DEVICE:
                                break;
                            case BackEndPhone.DOCTOR_CALL_DEVICE:
                                // doctor phone process assist call and emergency call
...
// callType new incoming call type
    public boolean CheckListenEnabled(BackEndPhone phone,int callType){
        boolean result = true;

        if(!phone.isReg)
            result = false;
        else {
            if (inviteCall.state==CommonCall.CALL_STATE_RINGING||inviteCall.state==CommonCall.CALL_STATE_DIALING||inviteCall.state==CommonCall.CALL_STATE_INCOMING){
                if(!CommonCall.IsAlertCall(inviteCall.callType)&&!CommonCall.IsAlertCall(callType)){
                    if(inviteCall.caller.compareToIgnoreCase(phone.id) == 0)
                        result = false;
                }
            }

//            if (inviteCall.state==CommonCall.CALL_STATE_RINGING||inviteCall.state==CommonCall.CALL_STATE_DIALING){
//                if(inviteCall.caller.compareToIgnoreCase(phone.id) == 0)
//                    result = false;
//            }else if(inviteCall.state==CommonCall.CALL_STATE_INCOMING){
...
//            }
    ...
    public boolean CheckInvitedEnable(BackEndPhone phone) {
        ...
        if(!phone.isReg)
            result = false;
        else if(!CommonCall.IsAlertCall(inviteCall.callType)){
            switch (phone.type) {
                case BackEndPhone.BED_CALL_DEVICE:
                    if (inviteCall.caller.compareToIgnoreCase(phone.id) == 0)
                        result = false;
                ...
                case BackEndPhone.NURSE_CALL_DEVICE:
                    break;
                case BackEndPhone.DOCTOR_CALL_DEVICE:  // doctor phone can be called directly
                    break;
                case BackEndPhone.CORRIDOR_CALL_DEVICE:
                case BackEndPhone.DOOR_CALL_DEVICE:
                case BackEndPhone.TV_CALL_DEVICE:
