            ...
            while(!isInterrupted()){
                ...
                try{
                    ...
                    if(recvPack.getLength()>0){
                        ...
                        if(packet!=null) {
                            if(HandlerMgr.UpdateBackEndDevSocket(packet.sender,udpServerSocket,recvPack.getAddress(),recvPack.getPort())){
                                ...
                                LogWork.Print(LogWork.BACKEND_NET_MODULE,LogWork.LOG_ERROR,"Server Recv And Discard Dev %s Data from %s:%d",packet.sender,recvPack.getAddress().getHostAddress(),recvPack.getPort());
                            }
                        }
                    }
                } catch (IOException e) {
