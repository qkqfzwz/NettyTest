...
import com.example.nettytest.terminal.terminalphone.BackupServer;
...
// for terminal Phone
    ...
    static public void CreateTerminalPhone(String id,int type,int netMode){
        terminalDevManager.AddDevice(id, PhoneParam.callServerAddress,netMode);

        TerminalPhone phone = new TerminalPhone(id,type);
        terminalPhoneMgr.AddDevice(phone);
    }

    static public void SaveBackupServer(String id,String address){
        NetDevice dev = terminalDevManager.GetDevice(id);
        if(dev.GetServerAddress().compareToIgnoreCase(PhoneParam.callServerAddress)==0){
            PhoneParam.SaveBackupServerAddress(address);
        }
    }

    static public void ActiveBackupServer(boolean status){
        if(!status){
            PhoneParam.callServerBackupSuspend = true;
        }else{
            PhoneParam.callServerBackupSuspend = false;
            HandlerMgr.ReadAreaInfo();
        }
    }

    static public void SwitchMainServer(String id){
        int netMode;
        boolean isSwitched = false;
        int devType;
        NetDevice dev = terminalDevManager.GetDevice(id);
        netMode = dev.GetNetMode();

        if(dev.GetServerAddress().compareToIgnoreCase(PhoneParam.callServerAddress)!=0){
            terminalDevManager.RemovePhone(id);
            terminalDevManager.AddDevice(id,PhoneParam.callServerAddress,netMode);
            isSwitched = true;
            LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_INFO,"Device %s Switch to Main Server");
        }

        if(isSwitched){
            TerminalPhone phone = terminalPhoneMgr.GetDevice(id);
            if(phone!=null)
                phone.ResetRegCount();
        }
    }

    static public void SwitchServer(String id){
        int netMode;
        boolean isSwitched = false;
        int devType;
        NetDevice dev = terminalDevManager.GetDevice(id);
        ...
        if(dev!=null){
            if(dev.GetServerAddress().compareToIgnoreCase(PhoneParam.callServerAddress)==0){
                if(!PhoneParam.callServerBackupAddress.isEmpty()){
                    terminalDevManager.RemovePhone(id);
                    terminalDevManager.AddDevice(id,PhoneParam.callServerBackupAddress,netMode);
                    isSwitched = true;
                    LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_INFO,"Device %s reg to Main server %s TimeOver, switch to backup Server %s", id,PhoneParam.callServerAddress,PhoneParam.callServerBackupAddress);
                }
            }else if(dev.GetServerAddress().compareToIgnoreCase(PhoneParam.callServerBackupAddress)==0){
                terminalDevManager.RemovePhone(id);
                terminalDevManager.AddDevice(id,PhoneParam.callServerAddress,netMode);
                isSwitched = true;
                LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_INFO,"Device %s reg to backup server %s TimeOver, switch to Main Server %s", id,PhoneParam.callServerBackupAddress,PhoneParam.callServerAddress);
            }else{
                LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_ERROR,"Server Address %s of Dev %s is not MainServer %s and Backup Server %s", dev.GetServerAddress(),id,PhoneParam.callServerAddress,PhoneParam.callServerBackupAddress);
            }
        }else{
            LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_ERROR,"Could Not find Dev %s in Net Dev List", id);
        }
    ...
    static public void CreateBackupServer(String id,int netMode){
        if(!PhoneParam.serverActive){
            terminalDevManager.AddDevice(id,PhoneParam.callServerAddress,netMode);

            BackupServer device = new BackupServer(id,TerminalPhone.SERVER_BACKUP_DEVICE);
            terminalPhoneMgr.AddDevice(device);
            UserInterface.StartServer();
            HandlerMgr.StartCheckDevices(PhoneParam.serviceAddress,PhoneParam.servicePort);
        }else{
            LogWork.Print(LogWork.DEBUG_MODULE,LogWork.LOG_ERROR,"Create Backup Server Fail, Call Server is Started as Main Server!!!!!!");
        }
    }

    static public void BackupAreaInfo(String areaId){
         if(DevicesQuery.devQueryCount>0)
             deviceQuery.BackupAreaInfo(areaId);
    }

    static public void ReadAreaInfo(){
        DevicesQuery.ReloadBackupServerInfo();
    }
...
// for backend TcpNetDevice
    ...
    static public void StopCheckDevices(){
        if(deviceQuery!=null){
            deviceQuery.StopQuery();
            deviceQuery = null;
        }
    }

    static public boolean UpdateBackEndDevChannel(String ID,Channel ch){
        return backEndDevMgr.UpdateDevChannel(ID,ch);
    }

    static public boolean CleanBackEndChannel(String ID,Channel ch) {
        return backEndDevMgr.CleanDevChannel(ID,ch);
    }

    static public boolean UpdateBackEndDevSocket(String ID, DatagramSocket socket,InetAddress hostAddr, int port){
        return backEndDevMgr.UpdateDevSocket(ID,socket,hostAddr,port);
    }
...
// for backend phone
    ...
    static public CallParams GegBackEndConfig(String areaId){
        return backEndPhoneMgr.GetAreaParams(areaId);
    }
    ...
    static public void UpdateAreaDevices(String areaId,ArrayList<UserDevice> devList) {
    	backEndPhoneMgr.UpdateAreaDevices(areaId,devList);
    }

    static public void UpdateAreaAlertConfig(String areaId,ArrayList<AlertConfig> configs){
        backEndPhoneMgr.UpdateAreaAlertConfig(areaId,configs);
    }

    static public ArrayList<AlertConfig> GetAreaAlertConfig(String id){
        return backEndPhoneMgr.GetAreaAlertConfig(id);
    }
...
// create backend device
    ...
    static public int  AddBackEndPhone(String ID,int type,int netMode,String area){
        ...
        if(result== FailReason.FAIL_REASON_NO)
            backEndDevMgr.AddDevice(ID,netMode,type);
        return result;
    }
    ...
    static public ArrayList<UserDevice> GetBackEndPhoneInfo(String areaId){
        ...
        ArrayList<BackEndPhone> phoneLists = backEndPhoneMgr.GetBackEndPhoneList(areaId);
        ArrayList<UserDevice> userListList = new ArrayList<>();
        for(BackEndPhone dev:phoneLists){
            ...
            userDev.bedName = dev.devInfo.bedName;
            userDev.devName = dev.devInfo.deviceName;
            userDev.areaId = dev.devInfo.areaId;
            userDev.areaName = dev.devInfo.areaName;
            userDev.roomName = dev.devInfo.roomName;
            userDev.roomId = dev.devInfo.roomId;
            userDev.netMode = dev.netMode;
