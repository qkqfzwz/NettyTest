            ...
            switch(type){
                ...
                case ProtocolPacket.REG_RES:
                    ...
                    if(context!=null){
                        ...
                        regResPack.backupServerAddress = JsonPort.GetJsonString(context,ProtocolPacket.PACKET_BACKUP_SERVER_ADDRESS_NAME);
                ...
                case ProtocolPacket.CALL_REQ:
                    ...
                    if(context!=null){
                        ...
                        if(CommonCall.IsAlertCall(inviteReqPack.callType)){
                            ...
                            inviteReqPack.nameType = context.getIntValue(ProtocolPacket.PACKET_NAMETYPE_NAME);
                        }
                    ...
                    if(context!=null){
                        ...
                        if(phoneLists!=null) {
                            for (int iTmp = 0; iTmp < phoneLists.size(); iTmp++) {
                                ...
                                UserDevice phone = new UserDevice();
                                phone.devid = JsonPort.GetJsonString(jsonObj,ProtocolPacket.PACKET_DEVID_NAME);
                                phone.type = jsonObj.getIntValue(ProtocolPacket.PACKET_DEVTYPE_NAME);
                                phone.isRegOk = jsonObj.getBooleanValue(ProtocolPacket.PACKET_STATUS_NAME);
                                ...
                                phone.areaId = JsonPort.GetJsonString(jsonObj,ProtocolPacket.PACKET_AREAID_NAME);
                                phone.areaName= JsonPort.GetJsonString(jsonObj,ProtocolPacket.PACKET_AREANAME_NAME);
                                phone.roomId= JsonPort.GetJsonString(jsonObj,ProtocolPacket.PACKET_ROOMID_NAME);
                                phone.roomName= JsonPort.GetJsonString(jsonObj,ProtocolPacket.PACKET_ROOMNAME_NAME);
                                phone.netMode = jsonObj.getIntValue(ProtocolPacket.PACKET_NETMODE_NAME);
                ...
                case ProtocolPacket.CALL_VIDEO_ANSWER_RES:
                    ...
                    p = answerVideoResP;
                    break;
                case ProtocolPacket.NOTIFY_SEND_REQ:
                    NotifyReqPack notifyReqP = new NotifyReqPack();
                    PutDefaultData(notifyReqP,json);
                    context = json.getJSONObject(ProtocolPacket.PACKET_CONTEXT_NAME);

                    notifyReqP.notifyType= context.getIntValue(ProtocolPacket.PACKET_NOTIFY_TYPE_NAME);
                    notifyReqP.notifyId = JsonPort.GetJsonString(context,ProtocolPacket.PACKET_NOTIFY_ID_NAME);
                    notifyReqP.notifyTitle= JsonPort.GetJsonString(context,ProtocolPacket.PACKET_NOTIFY_TITLE_NAME);
                    notifyReqP.notifyContent= JsonPort.GetJsonString(context,ProtocolPacket.PACKET_NOTIFY_CONTENT_NAME);
                    notifyReqP.notifySender= JsonPort.GetJsonString(context,ProtocolPacket.PACKET_NOTIFY_SENDER_NAME);
                    notifyReqP.notifyReceiver= JsonPort.GetJsonString(context,ProtocolPacket.PACKET_NOTIFY_RECEIVER_NAME);
                    p = notifyReqP;
                    break;
                case ProtocolPacket.NOTIFY_SEND_RES:
                    NotifyResPack notifyResP = new NotifyResPack();
                    PutDefaultData(notifyResP,json);
                    context = json.getJSONObject(ProtocolPacket.PACKET_CONTEXT_NAME);

                    notifyResP.notifyId = JsonPort.GetJsonString(context,ProtocolPacket.PACKET_NOTIFY_ID_NAME);
                    notifyResP.notifyReceiver = JsonPort.GetJsonString(context,ProtocolPacket.PACKET_NOTIFY_RECEIVER_NAME);
                    p = notifyResP;
                    break;
                case ProtocolPacket.SERVER_SUSPEND:
                    p = new ProtocolPacket();
                    break;
            }
            json.clear();
        ...
        try {
            ...
            switch(p.type){
                ...
                case ProtocolPacket.REG_RES:
                    ...
                    context.put(ProtocolPacket.PACKET_BACKUP_SERVER_ADDRESS_NAME,regResP.backupServerAddress);
                ...
                case ProtocolPacket.CALL_REQ:
                    ...
                    context.put(ProtocolPacket.PACKET_NAMETYPE_NAME,inviteReqP.nameType);
                    json.put(ProtocolPacket.PACKET_CONTEXT_NAME,context);
                    break;
                case ProtocolPacket.CALL_RES:
                ...
                case ProtocolPacket.DEV_QUERY_RES:
                    ...
                    for(UserDevice phone:devResP.phoneList){
                        JSONObject phoneJson = new JSONObject();
                        phoneJson.put(ProtocolPacket.PACKET_DEVID_NAME,phone.devid);
                        phoneJson.put(ProtocolPacket.PACKET_DEVTYPE_NAME,phone.type);
                        phoneJson.put(ProtocolPacket.PACKET_STATUS_NAME,phone.isRegOk);
                        ...
                        phoneJson.put(ProtocolPacket.PACKET_ROOMID_NAME,phone.roomId);
                        phoneJson.put(ProtocolPacket.PACKET_ROOMNAME_NAME,phone.roomName);
                        phoneJson.put(ProtocolPacket.PACKET_AREAID_NAME,phone.areaId);
                        phoneJson.put(ProtocolPacket.PACKET_AREANAME_NAME,phone.areaName);
                        phoneJson.put(ProtocolPacket.PACKET_NETMODE_NAME,phone.netMode);
                ...
                case ProtocolPacket.CALL_VIDEO_END_RES:
                    ...
                    break;
                case ProtocolPacket.NOTIFY_SEND_RES:
                    NotifyResPack notifyResP = (NotifyResPack)p;
                    context.put(ProtocolPacket.PACKET_NOTIFY_ID_NAME,notifyResP.notifyId);
                    context.put(ProtocolPacket.PACKET_NOTIFY_RECEIVER_NAME,notifyResP.notifyReceiver);
                    json.put(ProtocolPacket.PACKET_CONTEXT_NAME,context);
                    break;
                case ProtocolPacket.NOTIFY_SEND_REQ:
                    NotifyReqPack notifyReqP = (NotifyReqPack)p;
                    context.put(ProtocolPacket.PACKET_NOTIFY_TYPE_NAME,notifyReqP.notifyType);
                    context.put(ProtocolPacket.PACKET_NOTIFY_ID_NAME,notifyReqP.notifyId);
                    context.put(ProtocolPacket.PACKET_NOTIFY_TITLE_NAME,notifyReqP.notifyTitle);
                    context.put(ProtocolPacket.PACKET_NOTIFY_CONTENT_NAME,notifyReqP.notifyContent);
                    context.put(ProtocolPacket.PACKET_NOTIFY_SENDER_NAME,notifyReqP.notifySender);
                    context.put(ProtocolPacket.PACKET_NOTIFY_RECEIVER_NAME,notifyReqP.notifyReceiver);
                    json.put(ProtocolPacket.PACKET_CONTEXT_NAME,context);
                    break;
            }
        } catch (JSONException e) {
