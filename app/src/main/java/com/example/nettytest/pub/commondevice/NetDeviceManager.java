...
//                    dev.SendBuffer(bytebuf);
    ...
    public boolean UpdateDevChannel(String id, Channel ch){
        NetDevice matchedDev;
    ...
    public boolean CleanDevChannel(String id,Channel ch) {
        NetDevice matchedDev;
        boolean result = false;
        synchronized (NetDeviceManager.class) {
            matchedDev = devLists.get(id);
    ...
    public boolean UpdateDevSocket(String id, DatagramSocket ssocket, InetAddress hostAddress, int port){
        NetDevice matchedDev;
        boolean result = false;
        synchronized (NetDeviceManager.class) {
            matchedDev = devLists.get(id);
            if (matchedDev != null) {
                if(matchedDev.netType == NetDevice.UDP_NET_DEVICE) {
                    ...
                    result = dev.UpdatePeerAddress(ssocket,hostAddress, port);
                }
            }
        }
