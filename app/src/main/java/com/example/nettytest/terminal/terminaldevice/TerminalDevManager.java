...
public class TerminalDevManager extends NetDeviceManager {
    ...
    public void AddDevice(String id,String serverAddress,int netMode){
        ...
        synchronized (NetDeviceManager.class) {
            ...
            if(matchedDev==null) {
                if(netMode==PhoneParam.TCP_PROTOCOL) {
                    TerminalTcpDevice dev = new TerminalTcpDevice(id,serverAddress);
                    devLists.put(id, dev);
                    dev.Start();
                }else if(netMode == PhoneParam.RAW_TCP_PROTOCOL){
                    TerminalRawTcpDevice dev = new TerminalRawTcpDevice(id,serverAddress);
                    devLists.put(id,dev);
                }else if(netMode == PhoneParam.UDP_PROTOCOL){
                    TerminalUdpDevice dev = new TerminalUdpDevice(id,serverAddress);
                    devLists.put(id,dev);
                }
            }
        }
    ...
    public NetDevice GetDevice(String id){
        NetDevice matchedDev;
        synchronized (NetDeviceManager.class) {
            matchedDev = devLists.get(id);
        }
