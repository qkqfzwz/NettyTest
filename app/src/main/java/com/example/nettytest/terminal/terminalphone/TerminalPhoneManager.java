...
public class TerminalPhoneManager {
    ...
    int snapPort = -1;
    DatagramSocket snapSocket = null;
    ...
    public class SnapThread extends Thread {
        ...
        public SnapThread(int snap_port) {
            super("");
            snapPort = snap_port;
        }
        ...
        public void run() {
            ...
            byte[] snapResult;
            testSocket = SystemSnap.OpenSnapSocket(snapPort,PhoneParam.SNAP_TERMINAL_GROUP);
            DatagramPacket resPack;
            snapSocket = testSocket;
            int value;
            if (testSocket != null) {
...
//                                                LogWork.Print(LogWork.DEBUG_MODULE,LogWork.LOG_INFO,"Get Terminal Call Snap for dev %s, total %d bytes, send to %s:%d",devId,snapResult.length,recvPack.getAddress().getHostAddress(),recvPack.getPort());
    ...
    public int StartSnap(int port){
        if(snapThread==null){
            snapThread = new SnapThread(port);
            snapThread.start();
        }else if(port!=snapPort) {
            snapSocket.close();
            snapThread = new SnapThread(port);
            snapThread.start();
        }
    ...
    public void RemovePhone(String id){
        ...
        synchronized (TerminalPhoneManager.class) {
            ...
            if(matchedDev!=null){
                ...
                if(matchedDev.type==PhoneDevice.SERVER_BACKUP_DEVICE){
                    HandlerMgr.StopCheckDevices();
                }
            }
        }
    }
        ...
        if(phone!=null){
            switch(packet.type){
                case ProtocolPacket.NOTIFY_SEND_REQ:
                    phone.RecvNotify(packet);
                    LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_DEBUG,"DEV %s Recv Notify Send Req ",phone.id);
                    break;
                case ProtocolPacket.SERVER_SUSPEND:
                    HandlerMgr.SwitchMainServer(phone.id);
                    LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_DEBUG,"DEV %s Recv Server Suspend, Switch to Main Server ",phone.id);
                    break;
...
//                    phone.RecvUnsupport(packet);
            ...
            LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_DEBUG,"Could not Find Device %s!!!!!!!!!!!!!",packet.receiver);
        }
