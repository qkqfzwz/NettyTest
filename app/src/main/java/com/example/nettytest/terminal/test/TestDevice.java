...
//        UserInterface.PrintLog("Device List TextView Touch at (%d,%d)", x, y);
    ...
    //                            opResult = BuildAlert(1);
            ...
            }else if(type == UserInterface.CALL_NURSER_DEVICE||type==UserInterface.CALL_DOCTOR_DEVICE){
                if(selected==0){
            ...
            if(type==UserInterface.CALL_NURSER_DEVICE||
                    type==UserInterface.CALL_DOCTOR_DEVICE||
        ...
        synchronized(TestDevice.class){
            switch(msg.type){
                case UserAlertMessage.ALERT_MESSAGE_INCOMING:
                    ...
                    UserInterface.PrintLog("Recv Incoming Alert %s in Dev %s ,type=%d, voice=%s, display=%s",info.alertId,devid,msg.alertType,msg.voiceInfo,msg.displayInfo);
        ...
        synchronized (TestDevice.class) {
            switch (msg.type) {
                ...
                case UserCallMessage.CALL_MESSAGE_INCOMING:
                    ...
                    UserInterface.PrintLog("Recv Incoming Call %s , calltype =%d callerType=%d in Dev %s ",info.callID,msg.callType,msg.callerType,devid);
        ...
        if (devLists != null) {
            for (int iTmp = 0; iTmp < devLists.size(); iTmp++) {
                ...
                if(incomingRecordNum==null)
                    incomingRecordNum = 0;
                if (bedPhone.isRegOk) {
                    status += String.format("%s(%s) Register succ (%d)\n", bedPhone.devid,bedPhone.devName,incomingRecordNum);
//                    UserInterface.PrintLog("%s-%s Register Succ",bedPhone.devid,bedPhone.bedName);
                } else {
                    status += String.format("%s(%s) Register Fail (%d)\n", bedPhone.devid,bedPhone.devName,incomingRecordNum);
