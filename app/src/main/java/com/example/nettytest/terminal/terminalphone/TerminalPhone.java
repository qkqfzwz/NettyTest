...
import com.example.nettytest.pub.protocol.NotifyResPack;
...
public class TerminalPhone extends PhoneDevice {
    ...
    private boolean needSwitchMainServer;
    ...
    public int RecvNotify(ProtocolPacket packet){
        NotifyReqPack reqP = (NotifyReqPack)packet;
        NotifyResPack resP = new NotifyResPack(reqP);
        resP.notifyReceiver = id;

        Transaction notifyResTrans = new Transaction(id,reqP,resP,Transaction.TRANSCATION_DIRECTION_C2S);
        HandlerMgr.AddPhoneTrans(resP.msgID,notifyResTrans);
        if(reqP.notifyType== UserInterface.NOTIFY_MAIN_SERVER_ACTIVE){
    ...
    public String GetBackupServer(String address){
        return PhoneParam.callServerBackupAddress;
    }
    ...
    public void UpdateRegStatus(int status,RegResPack resP){
        ...
        if(status ==ProtocolPacket.STATUS_OK){
            ...
            regMsg.isReg = true;
            regMsg.backupServerAddress = resP.backupServerAddress;
            this.areaId = resP.areaId;
            isListenCall = resP.listenCallEnable;
            if(type<TERMINAL_DEVICE_MAX) {
                LogWork.Print(LogWork.TERMINAL_PHONE_MODULE, LogWork.LOG_DEBUG, "Phone %s Update Reg Status Success, BackupServer is %s", id, regMsg.backupServerAddress);
                HandlerMgr.SaveBackupServer(id, regMsg.backupServerAddress);
            }
            ...
            if(type<TERMINAL_DEVICE_MAX){
                LogWork.Print(LogWork.TERMINAL_PHONE_MODULE, LogWork.LOG_DEBUG, "Phone %s Update Reg TimerOver, Try to Switch Server ", id);
                HandlerMgr.SwitchServer(id);
            }
            
        }else{
            LogWork.Print(LogWork.TERMINAL_PHONE_MODULE,LogWork.LOG_WARN,"Phone %s Reg Fail for %s ",id,resP.result);
            isReg = false;
    ...
    public void UpdateDevLists(DevQueryResPack p){
        ...
        for(int iTmp=0;iTmp<p.phoneList.size();iTmp++){
            UserDevice pDevice = p.phoneList.get(iTmp);
            if(pDevice.devid.compareToIgnoreCase(id)==0)
                continue;
            if(pDevice.type == PhoneDevice.BED_CALL_DEVICE||pDevice.type== PhoneDevice.NURSE_CALL_DEVICE||pDevice.type== PhoneDevice.DOCTOR_CALL_DEVICE) {
                devsMsg.deviceList.add(pDevice);
            }
        }
    ...
    public boolean CheckAnsweredEnable(){
        ...
        if(type==BED_CALL_DEVICE||type==NURSE_CALL_DEVICE||type==DOCTOR_CALL_DEVICE){
            result = true;
        }
        return result;
    }
    ...
    public void UpdateSecondTick(){
        ...
        if(type<=TERMINAL_DEVICE_MAX){
            ...
            if(needSwitchMainServer){
                needSwitchMainServer = false;
                HandlerMgr.SwitchMainServer(id);
            }
        }
