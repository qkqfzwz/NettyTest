...
public class ClientTest {
    ...
    class ClientSnapThread extends Thread{
        ...
        public void run() {
            ...
            LogWork.Print(LogWork.TERMINAL_USER_MODULE,LogWork.LOG_ERROR,"Socket of ClientTest Snap Closed, Exit Snap Thread");
        }
    }
        ...
        if(PhoneParam.clientActive){
            ...
            synchronized(areaList){
                for(iTmp=0;iTmp<deviceNum;iTmp++){
                    ...
                    matchedArea = null;
                    if(dev.type!=UserInterface.CALL_SERVER_BACKUP_DEVICE){
                        ...
                        UserInterface.BuildDevice(dev.type,dev.devid,dev.netMode);
                        info.patientName = "backup_server"+dev.devid;
                        info.patientAge = String.format("%d",18);
                        UserInterface.SetDevInfo(dev.devid,info);
                  }
                }
            }
